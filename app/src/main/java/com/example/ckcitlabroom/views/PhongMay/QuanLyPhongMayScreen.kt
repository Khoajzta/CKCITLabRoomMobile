...
fun QuanLyPhongMay(
    ...
    val danhSachPhongMay =
        phongMayViewModel.danhSachAllPhongMay
            .filter { it.LoaiPhong == 1 || it.LoaiPhong == 2 }

    LaunchedEffect(Unit) { phongMayViewModel.getAllPhongMay() }
    DisposableEffect(Unit) {
        onDispose { phongMayViewModel.stopPollingPhongMay() }
    }
    ...
    Scaffold(
        ...
        floatingActionButton = {
            FloatingActionButtonCustom(
                onClick = { navController.navigate(NavRoute.ADDPHONGMAY.route) }
            )
        }
    ) { innerPadding ->
